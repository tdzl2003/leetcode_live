...
* 偏序优化：双指针，三指针。
* 矩阵，矩阵乘法：用矩阵乘法可快速线性递推式，尤其是二维线性递推式。有不少时候都可以通过母函数得到一个更好的解，但是矩阵方法简单易学。许多字符串问题可以通过状态机转变为线性变换问题求解。
...
**题意**： $1\times 2$的方块平铺$3\times N$的矩形，问有多少种平铺方案？
...
* 可以直接用母函数解出（注意上面博客链接里的计算过程有多处错误）：
